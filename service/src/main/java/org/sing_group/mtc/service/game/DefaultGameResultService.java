...
public class DefaultGameResultService implements GameResultService {
  @Inject
  ...
  public GameResult getResult(long id) {
    final GameResult gameResult = this.dao.get(id);
    
    return this.securityManager.ifAuthorized(
      checkThat.hasLogin(gameResult.getPatientLogin()),
      checkThat.hasLogin(gameResult.getTherapistLogin())
    ).call(() -> gameResult);
  }
}
